...
for iter in range(10):
        ...
        users = ["automoderator", "bot", "michaelm40", "kakrail69", "suitology"]
        disallowed = ["i'm a bot", "i am a bot", "your post has been removed", "live streaming free", "livestreaming free", "live stream free", "livestream free", "free livestream", "free live stream", "you have awarded karma", "sports live stream", "live stream online", "online freeports", "this post was made by", "polarized light neutron", "watch live", "click here", "http", "à", "å", "ä", "ç", "č", "é", "è", "í", "ñ", "ò", "ø", "ö", "ù", "ü", "ß"]
        words = defaultdict(int)
        prog = re.compile(r"(\s+|^\s*)((([A-Z]+)\s+\4\s+\4)|(([A-Z]+)\s+([A-Z]+)\s+([A-Z]+)\s+\6\s+\7)|(([A-Z]+)\s+([A-Z]+)\s+\10\s+\11))")
        for comment in all_comments:
                if comment.id not in already_done:
                        if not any(n in comment.author.name.lower() for n in users) and not any(i in comment.body.lower().encode('utf-8') for i in disallowed) and comment.body.count(" ") > 1 and not not isMod(comment):
